...
//        private uint _bucket_size;
        ...
        private ubyte index(const uint _rim) const pure
            in {
                assert(_rim <= rim);
            }
        do {
            if ( isBucket ) {
                ...
                return _archive.fingerprint[_rim];
            }
        }
...
//            assert( _buckets[pos].index != index );
        ...
        private void dump(string indent) const {
            ...
            writefln("<=|%s rim=%d", prefix.toHexString, rim);
        }
...
//                }
            ...
            if ( isBucket ) {
                ...
                writefln("add to bucke
t %s rim=%d %02x", archive.fingerprint.toHexString, rim,  archive.fingerprint[rim]);
...
//                     _buckets[0]._buckets=new Bucket[1];
// //                    _buckets[0]._buckets=new Bucket[1];
//                     _buckets[0]._buckets.add(_archive);
//                     _buckets[0]._buckets.add(archive);

//                    _buckets[0]._buckets[0]=sub_bucket;
                }
                else {
