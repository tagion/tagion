...
public class Base58 {
  ...
  // private static int[] INDICES = new int[128];
  // static this(){
  ...
  /+
    /**
     * Decodes the given base58 string into the original data ubytes, using the checksum in the
     * last 4 ubytes of the decoded data to verify that the rest are correct. The checksum is
     * removed from the returned data.
     *
     * @param input the base58-encoded string to decode (which should include the checksum)
     * @throws AddressFormatException if the input is not base 58 or the checksum does not validate.
     */
    public static ubyte[] decodeChecked(string input) {
        ubyte[] decoded  = decode(input);
        if (decoded.length < 4)
            throw new Exception("Input too short");
        ubyte[] data = decoded[0 .. decoded.length - 4];
        ubyte[] checksum = decoded[decoded.length - 4 .. decoded.length];
        ubyte[] actualChecksum = Arrays.copyOfRange(Sha256Hash.hashTwice(data), 0, 4);
