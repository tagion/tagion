...
struct Wallet(Net : SecureNet) {
    ...
    TagionCurrency getFee(TagionCurrency amount, bool print = false) {
        ...
        do {
            ...
            check((collected_bills.length != previous_bill_count) || !can_pay, format("Is unable to pay the amount %10.6fTGN available %10.6fTGN", amount_to_pay
    ...
    const(HiRPC.Sender) readIndicesByPubkey(HiRPC hirpc = HiRPC(null)) const {
        ...
        auto owner_indices = account.derivers.byKey
            .map!(owner => _net.hash.dartId(HashNames.trt_owner, owner));
    ...
    const(HiRPC.Sender) readContractsTRT(const(Document)[] contracts, HiRPC hirpc = HiRPC(null)) const {
        ...
        DARTIndex[] contract_indices = contracts.map!(doc => _net.hash.dartIndex(doc))
            .map!(idx => _net.hash.dartId(StdNames.contract, idx))
            .array;
