module tagion.crypto.secp256k1.c.secp256k1_musig;

import tagion.crypto.secp256k1.c.secp256k1;
import tagion.crypto.secp256k1.c.secp256k1_extrakeys;
import tagion.crypto.secp256k1.c.secp256k1_schnorrsig;
...
int secp256k1_musig_nonce_gen (
    ...
    const(ubyte)* session_id32,
    const(ubyte)* seckey,
    const(secp256k1_pubkey)* pubkey,
    const(ubyte)* msg32,
    const(secp256k1_musig_keyagg_cache)* keyagg_cache,
    const(ubyte)* extra_input32);
...
int secp256k1_musig_adapt (
    const(secp256k1_context)* ctx,
    ubyte* sig64,
    const(ubyte)* pre_sig64,
    const(ubyte)* sec_adaptor32,
    int nonce_parity);
