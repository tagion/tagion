module tagion.crypto.secp256k1.c.secp256k1_generator;
 ...
 */
extern __gshared const(secp256k1_generator)* secp256k1_generator_h;
...
int secp256k1_pedersen_verify_tally (
    ...
    size_t pcnt,
    const(secp256k1_pedersen_commitment*)* ncommits,
    size_t ncnt);
...
int secp256k1_pedersen_blind_generator_blind_sum (
    const(secp256k1_context)* ctx,
    const(ulong)* value,
    const(ubyte*)* generator_blind,
    ubyte** blinding_factor,
    size_t n_total,
    size_t n_inputs);
