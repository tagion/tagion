...
bool error_callback(
    ...
    CaseErrorCode:
        with (Document.Element.ErrorCode) {
            switch (error_code) {
                    ...
                    switch (current.type) {
                        ...
                        case E:
                            static if ((E is STRING) || (E is DOCUMENT) || (E is BINARY)) {
                                ...
                                const binary_len=LEB128.decode!uint(current.data[current.valuePos..$]);
                                writefln("\tbin_size %d", binary_len);
                                const buffer_pos=current.valuePos+binary_len.size;
                                writefln("\tbin_pos  %d", buffer_pos);
                                const buffer=current.data[buffer_pos..buffer_pos+binary_len.value];
                                hex_dump(buffer);
                            }
                                ...
                                static if (isIntegral!T) {
                                    const leb128_size=LEB128.calc_size(current.data[current.valuePos..$]);
                                    writefln("\tleb128  %d", leb128_size);
                                    hex_dump(current.data[current.valuePos..current.valuePos+leb128_size]);
                                }
                                else {
                                    hex_dump(current.data[current.valuePos..current.valuePos+T.sizeof]);
                                }
                            }
                            else static if (E is VER) {
                                const leb128_version_size=LEB128.calc_size(current.data[ubyte.sizeof..$]);
                                hex_dump(current.data[ubyte.sizeof..ubyte.sizeof+leb128_version_size]);
                            }
