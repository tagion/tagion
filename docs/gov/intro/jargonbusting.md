...
In many _blockchains_ systems, where _consensus mechanism_ and storage of data both happen "on the blocks", _finality_ entails that information (included in earlier blocks) can never be altered or deleted. This is different with Tagion because its _hashgraph consensus_ relates to the submitted changes to it's _database_, but functions independent, or outside of it. In this modular system, where _consensus_ and _storage_ are seperate, data can always be changed or deleted (given the correct permissions, of course), and _finality_ refers to reliability of each new _state_ of the _database_ without limiting data management and control.
