...
module tagion.hashgraph.HashGraph2;
...
@safe
class HashGraph2 : current_hashgraph.HashGraph {
 ...
 */
    ...
    do {
        if (valid_channel(event_pack.pubkey)) {
            auto event = new Event2(event_pack, this);
            _event_cache[event.fingerprint] = event;
        ...
        this(const Wavefront received_wave) pure nothrow {
         ...
         override current_event.Event lookup(const(Buffer) fingerprint) {
            ...
            if (fingerprint in event_package_cache) {
                ...
                if (valid_channel(event_pack.pubkey)) {
                    auto event = new Event2(event_pack, this.outer);
    ...
    version(none)
    package final Event2 register(const(Buffer) fingerprint) {
        if (_register) {
        ...
        foreach (fingerprint; super._register.event_package_cache.byKey) {
            ...
            if (registered_event.channel == from_channel) {
                if (front_seat_event is null) {
                    front_seat_event = cast(Event2)registered_event;
    ...
    do {
        ...
        foreach (epack; changes) {
            ...
            auto first_event = new Event2(epack, this);
    ...
    class Node {
        ...
        /++
         ...
         +/
        private void front_seat(Event2 event)
        in {
        ...
        private Event2 _event; /// This is the last event in this Node
