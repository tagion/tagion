...
import tagiom.wasm.executor.Executor;
...
Expect<void> runExpression(ref StackManager StackMgr) {
...
Expect<void>
runFunction(Range)(
    ref StackManager StackMgr,
    ref const(FunctionInstance) Func,
    Range Params) if (isIntputRange!Range && is(ElementType!Range : const(ValVariant))) {
  // Set start time.
    ...
    //     StackMgr.push(Val);
    // }
    Params.each!((p) => StackMgr.push(a));

  // Enter and execute function.
  AST::InstrView::iterator StartIt;
  {
  auto Res = enterFunction(StackMgr, Func, Func.getInstrs().end());
  if (Res) {
