    ...
    const(HRPCSender) opCall(ref const(HRPCReceiver) received) {
        if ( !received.params.empty ) {
            if ( received.params.hasElement("id") ) {
                immutable message=format("The parameter 'id' should be called 'account' instead", received.message.method);
        ...
        for (size_t i = 0; i < reads.length; i++) {
            if (socketSet.isSet(reads[i])) {
                ...
                if (datLength == Socket.ERROR) {
                    ...
                    writefln("\nReceived %d bytes from %s", datLength, reads[i].remoteAddress().toString());
