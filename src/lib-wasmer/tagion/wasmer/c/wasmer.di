...
enum WASMER_VERSION = "4.2.7";
enum WASMER_VERSION_MAJOR = 4;
enum WASMER_VERSION_MINOR = 2;
enum WASMER_VERSION_PATCH = 7;
enum WASMER_VERSION_PRE = "";
...
alias wasmer_metering_cost_function_t = c_ulong function (wasmer_parser_operator_t wasm_operator);
...
bool wasi_config_mapdir (wasi_config_t* config, const(char)* alias_, const(char)* dir);

wasi_config_t* wasi_config_new (const(char)* program_name);

bool wasi_config_preopen_dir (wasi_config_t* config, const(char)* dir);

void wasi_env_delete (wasi_env_t* state);

bool wasi_env_initialize_instance (
    wasi_env_t* wasi_env,
    wasm_store_t* store,
    wasm_instance_t* instance);

wasi_env_t* wasi_env_new (wasm_store_t* store, wasi_config_t* config);
...
wasi_env_t* wasi_env_with_filesystem (
    wasi_config_t* config,
    wasm_store_t* store,
    const(wasm_module_t)* module_,
    const(wasi_filesystem_t)* fs,
    wasm_extern_vec_t* imports,
    const(char)* package_);

void wasi_filesystem_delete (wasi_filesystem_t* ptr);

wasi_filesystem_t* wasi_filesystem_init_static_memory (const(wasm_byte_vec_t)* volume_bytes);

bool wasi_get_imports (
    const(wasm_store_t)* _store,
    wasi_env_t* wasi_env,
    const(wasm_module_t)* module_,
    wasm_extern_vec_t* imports);
...
bool wasi_get_unordered_imports (
    wasi_env_t* wasi_env,
    const(wasm_module_t)* module_,
    wasmer_named_extern_vec_t* imports);

wasi_version_t wasi_get_wasi_version (const(wasm_module_t)* module_);
...
bool wasmer_cpu_features_add (
    wasmer_cpu_features_t* cpu_features,
...
wasmer_features_t* wasmer_features_new ();
...
void wasmer_funcenv_delete (wasmer_funcenv_t* _funcenv);

wasmer_funcenv_t* wasmer_funcenv_new (wasm_store_t* store, void* data);

bool wasmer_is_compiler_available (wasmer_compiler_t compiler);

bool wasmer_is_engine_available (wasmer_engine_t engine);

bool wasmer_is_headless ();
...
ulong wasmer_metering_get_remaining_points (wasm_instance_t* instance);

wasmer_metering_t* wasmer_metering_new (
    ulong initial_limit,
    wasmer_metering_cost_function_t cost_function);

bool wasmer_metering_points_are_exhausted (wasm_instance_t* instance);

void wasmer_metering_set_remaining_points (wasm_instance_t* instance, ulong new_limit);

void wasmer_module_name (const(wasm_module_t)* module_, wasm_name_t* out_);

wasm_module_t* wasmer_module_new (wasm_engine_t* engine, const(wasm_byte_vec_t)* bytes);

bool wasmer_module_set_name (wasm_module_t* module_, const(wasm_name_t)* name);
...
void wasmer_named_extern_vec_copy (
    wasmer_named_extern_vec_t* out_ptr,
    const(wasmer_named_extern_vec_t)* in_ptr);

void wasmer_named_extern_vec_delete (wasmer_named_extern_vec_t* ptr);

void wasmer_named_extern_vec_new (
    wasmer_named_extern_vec_t* out_,
    uintptr_t length,
    wasmer_named_extern_t** init);

void wasmer_named_extern_vec_new_empty (wasmer_named_extern_vec_t* out_);

void wasmer_named_extern_vec_new_uninitialized (
    wasmer_named_extern_vec_t* out_,
    uintptr_t length);

void wasmer_target_delete (wasmer_target_t* _target);

wasmer_target_t* wasmer_target_new (
...
wasmer_triple_t* wasmer_triple_new_from_host ();

const(char)* wasmer_version ();

ubyte wasmer_version_major ();

ubyte wasmer_version_minor ();

ubyte wasmer_version_patch ();

const(char)* wasmer_version_pre ();

void wat2wasm (const(wasm_byte_vec_t)* wat, wasm_byte_vec_t* out_);
