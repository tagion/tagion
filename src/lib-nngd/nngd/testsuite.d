...
version (unittest) {
    ...
    static string nngtest_socket_properties(ref NNGSocket s, string tag) {
        ...
        if (s.errno != 0)
            res ~= format("\t\t== state: %d errno: %d errstr: %s\n", s.state, s.errno, s.errno);
        res ~= format("\tname       :\t %s\n ", s.name);
        if (s.errno != 0)
            res ~= format("\t\t== state: %d errno: %d errstr: %s\n", s.state, s.errno, s.errno);
        res ~= format("\traw        :\t %s\n ", s.raw);
    ...
    static uint rot3(uint data) {
        return (data << 3) & (data >> 29);
    }

    static uint mkrot3(uint data) {
        return data ^ rot3(data);
    }

    static bool chkrot3(uint data, uint chk) {
        return (chk ^ rot3(data)) == data;
    }
    ...
    @trusted class NNGTest {
        void log(A...)(string fmt, A a) @trusted {
            if (!(flags & nngtestflag.DEBUG) || logfile is null)
                return;
        ...
        void seterrors(string[] e) @trusted {
            _errors ~= e;
        }
        ...
        override string[] run() @trusted {
            ...
            static foreach (i, t; nngd.nngtests.testlist) {
                mixin(
                        ...
                        "auto t" ~ to!string(
                            i) ~ " = new " ~ t ~ "(this.logfile, this.flags); \n" ~
                        "this.tests[\"" ~ to!string(
                            i) ~ "\"] = cast(NNGTest*)&(t" ~ to!string(i) ~ "); \n" ~
                        "auto task" ~ to!string(
                            i) ~ " = task(&(t" ~ to!string(i) ~ ".run)); \n" ~
            ...
            static foreach (i, t; nngd.nngtests.testlist) {
                mixin(
                        "if( this.todo < 0 || this.todo == " ~ to!string(
                        i) ~ " ) { \n" ~
