    ...
    struct RecycleIndices {
        ...
        void dump() {
            ...
            if ( !s.empty ) {
                ...
                writefln("s.back.end_index=%d owner.last_block_index=%d s.end_index=%d s=%s back=%s", s.back.end_index, owner.last_block_index, s.front.end_index, s, s.back);
        ...
        const(uint) reserve_segment(bool random=false)(const uint size) {
            ...
            do {
                ...
                if ( size < segment_to_be_removed.size ) {
                    recycle_segments.insert(Segment(segment_to_be_removed.begin_index+size, segment_to_be_removed.end_index));
                }
...
//                            assert(found.size > 0);
                            .check(found.end_index < owner.last_block_index, format("recylce blocks=%d extends beond last_block_index=%d", found.end_index, owner.last_block_index));
            ...
            if ( owner.last_block_index > 1 ) {
                ...
                if ( end_of_blocks_index in blocks ) {
                    ...
                    if ( end_block.next !is INDEX_NULL )  {
                        blocks[end_of_blocks_index]=owner.block(end_block.previous, INDEX_NULL, end_block.size, end_block.data, end_block.head);
                    }
                }
            }
        }
        ...
        bool check() pure const {
            ...
            scope recycle_sorted_tree=redBlackTree!("a.begin_index < b.begin_index")(recycle_segments[]);
    ...
    void createHeader(string name) {
        ...
        check(name.length < headerblock.id.length, format("Id is limited to a length of %d but is %d", headerblock.id.length, name.length));
    ...
    static class Block {
        ...
        private this(immutable uint previous, immutable uint next, immutable uint size, immutable(Buffer) buf, const bool head) {
    ...
    final Block block(immutable uint previous, immutable uint next, immutable uint size, immutable(Buffer) buf, const bool head)
        in {
    ...
    string toInfo(const Block block) const {
        with(block) {
            return format("<-[%04d] ->[%04d] blocks=%d size=%d head=%s", previous, next, number_of_blocks(size), size, head);
        }
    }
    ...
    private void readHeaderBlock() {
        check(file.size % BLOCK_SIZE == 0,
            format("BlockFile should be sized in equal number of blocks of the size of %d but the size is %d", BLOCK_SIZE, file.size));
        last_block_index=cast(uint)(file.size / BLOCK_SIZE);
        check(last_block_index > 1, format("The BlockFile should at least have a size of two block of %d but is %d", BLOCK_SIZE, file.size));
    ...
    immutable(Buffer) load(const uint index) {
        ...
        @safe void build_sequency(scope const Block block, ubyte[] cache) {
            if ( block.size > DATA_SIZE ) {
                ...
                check(!next_block.head, format("Block @ index %d is marked as head of block sequency but it should not be", index));
...
//        check(index !is INDEX_NULL, "Block zero can not be ereased");
    ...
    class AllocatedChain {
        ...
        string toInfo() const {
            return format("[%d..%d] blocks=%s size=%5d", chain.begin_index, end_index, number_of_blocks(size), size);
        }
    final:
    ...
    void store() {
        ...
        void allocate_and_chain(SortedSegments)(const(AllocatedChain[]) allocate, ref scope SortedSegments sorted_segments) @safe {
            if ( allocate.length > 0 ) {
                uint chain(immutable(ubyte[]) data, const uint current_index, const uint previous_index, const bool head) @trusted {
                    ...
                    if ( data !is null ) {
                        ...
                        if ( data.length > DATA_SIZE ) {
                            void update_first_index(uint current_index){
                                if ( (masterblock.first_index > current_index) || (masterblock.first_index is INDEX_NULL) ) {
                            ...
                            while(from + DATA_SIZE < data.length){
                                ...
                                blocks[current]=block(previous, next_index, cast(uint)(data.length - from), slice_data, h);
                    ...
                    if ( !sorted_segments.empty && (current_index is sorted_segments.front.begin_index) ) {
                ...
                if ( !sorted_segments.empty  && (sorted_segments.front.end_index < ablock.begin_index ) ) {
                    ...
                    if ( end_block.previous !is previous_index ) {
                        blocks[current_segment.end_index]=block(previous_index, end_block.next, end_block.size, end_block.data, end_block.head);
                    }
                    sorted_segments.popFront;
                    allocate_and_chain(allocate, sorted_segments);
                }
                else {
                    if ( !sorted_segments.empty && (sorted_segments.front.end_index is ablock.begin_index) ) {
        ...
        scope segments_needs_saving=array(recycle_indices.update_segments[]).sort!(q{a.end_index < b.begin_index});
        if ( !segments_needs_saving.empty && (segments_needs_saving[$-1].end_index >= last_block_index) ) {
...
//        console.writefln("owner.last_block_index=%d", last_block_index);
    ...
    void dump() {
        ...
        foreach(index;0..((last_block_index / block_per_line)+((last_block_index % block_per_line==0)?0:1))*block_per_line) {
