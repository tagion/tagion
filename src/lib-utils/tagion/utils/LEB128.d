...
@safe @nogc class LEB128Exception : TagionException
{
...
+/
@safe @nogc size_t calc_size(const(ubyte[]) data) pure nothrow
{
    foreach (i, d; data)
    {
        if ((d & 0x80) == 0)
...
@safe @nogc size_t calc_size(T)(const T v) pure nothrow if (isUnsigned!(T))
{
    size_t result;
    ulong value = v;
    do
...
@safe @nogc size_t calc_size(T)(const T v) pure nothrow if (isSigned!(T))
{
    if (v == T.min)
    {
...
@safe immutable(ubyte[]) encode(T)(const T v) pure 
...
@safe immutable(ubyte[]) encode(T)(const T v) pure if (isSigned!T && isIntegral!T)
{
...
+/
@safe @nogc DecodeLEB128!T decode(T = ulong)(const(ubyte[]) data) pure nothrow 
        if (isUnsigned!T)
{
    alias BaseT = TypedefType!T;
    ulong result;
...
/++
 ...
 In case of an error this size is set to zero
+/
@safe @nogc DecodeLEB128!T decode(T = long)(const(ubyte[]) data) pure nothrow 
        if (isSigned!T)
{
    alias BaseT = TypedefType!T;
    long result;
