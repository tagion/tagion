...
@nogc @safe uint gene_count(const size_t bitstring) pure nothrow
{
...
@trusted immutable(ulong[]) gene_xor(scope const(ulong[]) a, scope const(ulong[]) b) pure nothrow
in
{
...
@nogc @safe void gene_xor(ref scope ulong[] result, scope const(ulong[]) a, scope const(ulong[]) b) pure nothrow
in
{
    assert(a.length == b.length);
