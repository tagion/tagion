...
module tagion.network.wolfssl.c.ssl_mod;
...
struct WOLFSSL_STACK_;
alias WOLFSSL_LHASH = WOLFSSL_STACK_;
...
alias WOLFSSL_STACK = WOLFSSL_STACK_;
struct WOLFSSL_X509;
struct WOLFSSL_X509_NAME;
struct WOLFSSL_X509_NAME_ENTRY;
struct WOLFSSL_X509_CHAIN;
struct WC_PKCS12_;
alias WOLFSSL_X509_PKCS12 = WC_PKCS12_;
...
alias wolfSSL_Mutex = int;
alias word16 = ushort;

struct WOLFSSL_EVP_PKEY_
{
    import std.bitmanip : bitfields;
...
alias WOLFSSL_EVP_PKEY = WOLFSSL_EVP_PKEY_;
...
enum SNICbReturn
{
    warning_return = AlertLevel.alert_warning,
    fatal_return = AlertLevel.alert_fatal,
    noack_return = fatal_return + warning_return
}
...
WOLFSSL_METHOD* wolfTLS_client_method () @trusted;

WOLFSSL_METHOD* wolfTLS_server_method_ex (void* heap);
WOLFSSL_METHOD* wolfTLS_server_method () @trusted;
...
WOLFSSL_CTX* wolfSSL_CTX_new (WOLFSSL_METHOD* method) @trusted;
int wolfSSL_CTX_up_ref (WOLFSSL_CTX* ctx);

WOLFSSL* wolfSSL_new (WOLFSSL_CTX* ctx) @trusted;
...
int wolfSSL_set_fd (WOLFSSL* ssl, int fd) @trusted;
...
int wolfSSL_connect (WOLFSSL* ssl) @trusted;
int wolfSSL_write (WOLFSSL* ssl, const(void)* data, int sz);
int wolfSSL_read (WOLFSSL* ssl, void* data, int sz);
int wolfSSL_peek (WOLFSSL* ssl, void* data, int sz);
int wolfSSL_accept (WOLFSSL* ssl) @trusted;
...
/* WOLFSSL_EARLY_DATA */
/* WOLFSSL_TLS13 */
void wolfSSL_CTX_free (WOLFSSL_CTX* ctx) @trusted;
void wolfSSL_free (WOLFSSL* ssl) @trusted;
...
int wolfSSL_get_error (const WOLFSSL* ssl, int ret) @trusted;
...
void wolfSSL_CTX_set_verify (
    WOLFSSL_CTX* ctx,
    int mode,
    VerifyCallback verify_callback) @trusted;

void wolfSSL_set_verify (WOLFSSL* ssl, int mode, VerifyCallback verify_callback) @trusted;
void wolfSSL_set_verify_result (WOLFSSL* ssl, c_long v);

void wolfSSL_SetCertCbCtx (WOLFSSL* ssl, void* ctx);
void wolfSSL_CTX_SetCertCbCtx (WOLFSSL_CTX* ctx, void* userCtx);

int wolfSSL_pending (WOLFSSL* ssl) @trusted;
...
alias WOLFSSL_GENERAL_NAMES = WOLFSSL_STACK_;
alias WOLFSSL_DIST_POINTS = WOLFSSL_STACK_;
...
void wolfSSL_ERR_print_errors_fp (FILE* fp, int err = 0) @trusted;
...
void wolfSSL_ERR_put_error (
    ...
    const(char)* file,
    int line);
c_ulong wolfSSL_ERR_get_error_line (const(char*)* file, int* line);
c_ulong wolfSSL_ERR_get_error_line_data (
    const(char*)* file,
    int* line,
    const(char*)* data,
    int* flags);
@trusted
{
    void wolfSSL_AES_cbc_encrypt(const char*, char*, size_t, const void*, char*, const int);
    c_ulong wolfSSL_ERR_get_error ();
    void wolfSSL_ERR_clear_error ();
    int wolfSSL_AES_set_encrypt_key(const ubyte* key, const int bits, void* aes);
    int wolfSSL_AES_set_decrypt_key(const ubyte* key, const int bits, void* aes);
}
...
int wolfSSL_CTX_check_private_key (const(WOLFSSL_CTX)* ctx) @trusted;
...
int wolfSSL_state (const WOLFSSL* ssl);
...
int wolfSSL_writev (WOLFSSL* ssl, const(iovec)* iov, int iovcnt);
