...
//            writefln("node_name=%s", node_name, );
    ...
    foreach (i; 0..opts.nodes) {
        ...
        if(opts.dart.path.indexOf(dir_token) != -1){
            ...
            import std.path;
            if(!is_master_node){
                sync_opts.dart.path = stripExtension(opts.dart.path) ~ to!string(i) ~ extension(opts.dart.path);
            }
        }
        sync_opts.dart.task_name = opts.dart.task_name~to!string(i);
        sync_opts.dart.sync.task_name = opts.dart.sync.task_name~to!string(i);
        sync_opts.dart.mdns.task_name = opts.dart.mdns.task_name~to!string(i);
