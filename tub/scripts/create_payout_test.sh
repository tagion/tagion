...
usage() { echo "Usage: $0 -b <bindir> [-n <nodes=$nodes>] [-w <wallets=$wallets>] [-q <bills=$bills>] [-a <amount = $amount>] [-k <network dir = $ndir>] [-t <wallets dir = $wdir>] [-u <key filename=$keyfile]" 1>&2; exit 1; }

while getopts "n:w:a:b:k:t:h:u:q:" opt
do
    case $opt in
        h)  usage ;;
        n)  nodes=$OPTARG ;;
