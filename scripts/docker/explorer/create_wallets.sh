...
wallets=5
bills=50
ndir=$(realpath  "./network")
wdir=$(realpath  "./wallets")
keyfile=$(realpath  "./keys")
...
do
    case $opt in
        ...
        n)  nodes=$OPTARG ;;
        w)  wallets=$OPTARG ;;
        b)  bdir=$(realpath  "$OPTARG") ;;
        k)  ndir=$(realpath  "$OPTARG") ;;
        t)  wdir=$(realpath  "$OPTARG") ;;
        u)  keyfile=$(realpath  "$OPTARG") ;;
        q)  bills=$OPTARG ;;
        *)  usage ;;
    esac
done
...
rm -f "$keyfile" || echo "No key file to delete"
touch $keyfile
...
do
  ...
  wallet_dir=$(realpath   "${wdir}/wallet$i")
  mkdir -p $wallet_dir
  wallet_config=$(realpath   "${wdir}/wallet$i.json")
  password="password$i"
  ...
  for (( b=1; b <= bills; b++)); 
  do
    bill_name=$(realpath  "$wdir/bill$i-$b.hibon")
