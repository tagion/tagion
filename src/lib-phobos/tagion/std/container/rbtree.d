...
*/
module tagion.std.container.rbtree;
                    ...
                    if (cur._parent.isLeftNode) {
                        ...
                        if (y !is null && y.color == Color.Red) {
                            ...
                            if (cur._parent is end) {
                                ...
                                // not root node
                                cur.color = Color.Red;
                                if (cur._parent.color == Color.Black) // satisfied, exit the loop
                                    break;
                            }
                        }
                        else {
...
final class RedBlackTree(T, alias less = "a < b", bool allowDuplicates = false)
    ...
    override bool opEquals(Object rhs) {
        ...
        auto thisRange = this[];
        auto thatRange = that[];
        return equal!((Elem a, Elem b) => !_less(a, b) && !_less(b, a))(thisRange, thatRange);
    }
    ...
    override size_t toHash() nothrow @safe {
        ...
        foreach (ref e; this[]) // As in boost::hash_combine
     ...
     */
    size_t stableInsert(Stuff)(Stuff stuff) if (isInputRange!Stuff &&
...
--------------------
    ...
    /++ Ditto +/
    size_t removeKey(U)(scope U[] elems) if (isImplicitlyConvertible!(U, Elem)) {
        immutable lenBefore = length;

        foreach (e; elems) {
            auto beg = _firstGreaterEqual(e);
            if (beg is _end || _less(e, beg.value)) // no values are equal
                continue;
...
/++ Ditto +/
auto redBlackTree(alias less, E)(E[] elems...) if (is(typeof(binaryFun!less(E.init, E.init)))) {
...
/++ Ditto +/
auto redBlackTree(Stuff)(Stuff range) if (isInputRange!Stuff && !isArray!(Stuff)) {
    return new RedBlackTree!(ElementType!Stuff)(range);
}

/++ Ditto +/
auto redBlackTree(bool allowDuplicates, Stuff)(Stuff range) if (isInputRange!Stuff && !isArray!(Stuff)) {
