...
@safe class LRU(K, V)
{
    ...
    {
        static assert(hasMember!(V, "undefined"),
                format("%s must have a static member named 'undefined'", V.stringof));
    }
    ...
    @nogc bool contains(scope const(K) key) const pure nothrow
    {
    ...
    static if (does_not_have_immutable_members)
    {
        @nogc bool peek(const(K) key, ref V value) pure nothrow
        {
            auto ent = key in items;
            if (ent !is null)
            {
    ...
    @nogc void setEvict(EvictCallback evict) nothrow
    {
        onEvict = evict;
    }
    ...
    @nogc const(Entry)* getOldest() const pure nothrow
    {
...
unittest
{
    ...
    foreach (i, k; l.keys.enumerate)
    {
        ...
        bool not_good = ((i < amount_i) && (k != i + amount2 + 1))
            || ((i == amount_i) && (k != amount2));
