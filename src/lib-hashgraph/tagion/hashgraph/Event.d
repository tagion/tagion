   ...
   */
     ...
     */
        ...
        void vote(ulong vote_node_id, HashGraph hashgraph) {
            ...
            if (hashgraph._rounds.voting_round_per_node.all!(r => r.number >= current_round.number)) {
  ...
  */
    ...
    */
    ...
    void calc_youngest_ancestors(HashGraph hashgraph) {
        ...
        _youngest_ancestors = (father.round is mother.round) ? _mother._youngest_ancestors.dup() : new Event[hashgraph
                .node_size];
        ...
        iota(_father._youngest_ancestors.length)
            ...
            .filter!((ulong i) => _youngest_ancestors[i] is null || _father._youngest_ancestors[i].received_order > _youngest_ancestors[i]
            .received_order)
    ...
    void calc_vote(HashGraph hashgraph, ulong vote_node_id) {
        ...
        if (voting_round.number >= round.number) {
            return;
        }
