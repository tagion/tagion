...
struct AESCrypto(int KEY_LENGTH) {
    ...
    const(ubyte[AES_BLOCK_SIZE]) Iv;
    @disable this();
    this(const(ubyte[]) iv) in {
        Iv=iv[0..AES_BLOCK_SIZE];
    }

    size_t enclength(const size_t inputlength) {
    ...
    version(TINY_AES) {
        ...
        alias AES=Tiny_AES!KEY_LENGTH;
        enum AES_BLOCK_SIZE=AES.BLOCKLEN;
        enum KEY_SIZE=AES.KEY_SIZE;
//        alias enclength=enclengthT!AES_BLOCK_SIZE;
//        enum AES_BLOCK_SIZE=16;
        ...
        do {
            scope aes=Tiny_AES!KEY_LENGTH(key[0..KEY_SIZE]);
            static if (ENCRYPT) {
                aes.CBC_encrypt(data, iv);
            }
            else {
                aes.CBC_decrypt(data. iv);
            }
        }

        void crypt_parse(bool ENCRYPT=true)(const(ubyte[]) key, const(ubyte[]) indata, ref ubyte[] outdata)
            in {
                if (outdata !is null) {
        ...
        do {
            ...
            size_t old_length;
            if (outdata.length % AES_BLOCK_SIZE !is 0) {
        ...
        void crypt(bool ENCRYPT=true)(const(ubyte[]) key, const(ubyte[]) indata, ref ubyte[] outdata) @trusted
            in {
                assert(indata);
                if (outdata !is null) {
...
//        scope ubyte[KEY_SIZE] mem_key;
            scope ubyte[AES_BLOCK_SIZE] mem_iv=iv;
//            auto iv=mem_iv;
            AES_KEY crypt_key;
            if ( outdata is null ) {
...
//                     0x08, 0xb0, 0xe2, 0x79, 0x88, 0x59, 0x88, 0x81, 0xd9, 0x20, 0xa9, 0xe6, 0x4f, 0x56, 0x15, 0xcd ];
            ...
            ubyte[] enc_output;
            AESCrypto.encrypt(key, indata, enc_output);
