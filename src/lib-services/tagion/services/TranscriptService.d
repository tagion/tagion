...
@safe struct TrascriptServiceTask
{
    import tagion.tasks.TaskWrapper;
    mixin TaskBasic;

    uint current_epoch;
    const StdSecureNet net;// = new StdSecureNet; //*/
    RecordFactory rec_factory;
    const HiRPC empty_hirpc = HiRPC(null);
    Tid dart_tid;
    SmartScript[Buffer] smart_scripts;
    RecordFactory.Recorder input_recorder;
    string task_name, dart_task_name;
    ...
    bool to_smart_script(ref const(SignedContract) signed_contract) nothrow @trusted
    {
        try
        {
    ...
    @TaskMethod void receive_epoch(Buffer payloads_buff) nothrow @trusted
    {
        try
        {
    ...
    {
        this.task_name = opts.transcript.task_name;
        this.dart_task_name = opts.dart.sync.task_name;
        this.rec_factory = RecordFactory.opCall(net);
        dart_tid = locate(dart_task_name);
