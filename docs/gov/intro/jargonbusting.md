...
The second _protocol_ then establishes _consensus_ not by comparing the databases themselves (or counting the number of blocks in a chain) but by tracking the level of shared knowledge that _nodes_ have communicated about in the wavefront protocol.<code style="color:red">[/to be re-written] This _consensus protocol_ was originally developed and patented by Leemon Baird (co-founder of the "Hedera" project), but released under an open-source license and called the "[Hashgraph consensus algorithm](https://docs.tagion.org/tech/protocols/consensus/HashGraph)".[to be re-written]</code>
