        ...
        ArchiveTab find(immutable(ubyte[]) key) {
            ...
            if ( isBucket ) {
                ...
                if ( (pos >= 0) && (pos < _bucket_size) && _buckets[pos] ) {
                    writefln("\t\trim=%d key=%s", _buckets[pos].rim, key[_buckets[pos].rim]);
...
//                    _buckets.insertInPlace(insert_pos, new Bucket(archive, _rim));
...
//                    temp_bucket;
            ...
            bool same_index(immutable int pos, immutable ubyte _index) {
                if ( (pos >= 0) && (pos < _bucket_size) ) {
                    return _buckets[pos].index(_rim) == _index;
                }
                return false;
            }
            ...
            if ( isBucket ) {
                ...
                immutable index=archive.fingerprint[_rim];
                immutable pos=find_bucket_pos(index);
                if ( same_index(pos, index) ) {
                    writefln("Same index");
                    _buckets[pos].add(archive, _rim+1);
                }
                else {
                    insert(pos, archive);
                }
                ...
                writefln("add to bucket %s rim=%d %02x", archive.fingerprint.toHexString, _rim,  archive.fingerprint[_rim]);
                writefln("............. %s rim=%d %02x", _archive.fingerprint.toHexString, _rim,  _archive.fingerprint[_rim]);
