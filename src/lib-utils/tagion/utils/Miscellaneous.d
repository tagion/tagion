...
@trusted string toHexString(bool UCASE = false, BUF)(BUF buffer) pure nothrow 
        if (isBufferType!BUF)
{
    static if (UCASE)
    {
...
+/
@safe string cutHex(bool UCASE = false, BUF)(BUF buf) pure if (isBufferType!BUF)
{
    import std.format;
...
@safe Buffer xor(const(ubyte[]) a, const(ubyte[]) b) pure nothrow
in
{
...
@nogc @safe void xor(ref scope ubyte[] result, scope const(ubyte[]) a, scope const(ubyte[]) b) pure nothrow
in
{
    assert(a.length == b.length);
    assert(a.length % ulong.sizeof == 0);
...
@safe Buffer xor(Range)(Range range) pure
{
