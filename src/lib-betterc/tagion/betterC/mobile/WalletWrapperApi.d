...
/// Functions called from d-lang through dart:ffi
    ...
    ResultDoc toResultDoc(Document doc){
        ResultDoc result;
        auto buffer = doc.serialize;
        result.ptr = cast(ubyte*) buffer.ptr;;
        result.len = buffer.length;
        return result;
    }
    Document fromResultDoc(ResultDoc resultDoc){
        Buffer result = cast(immutable)(resultDoc.ptr[0 .. resultDoc.len]);
        return Document(result);
    }

    export ResultDoc wallet_create(const uint8_t* pincodePtr, const uint32_t pincodeLen, 
        ...
        const uint32_t answerslen, uint32_t confidence)
    {
        ...
        auto wallet = SecureWallet!(SecureNet).createWallet(questions,
            answers, confidence, cast(immutable(char)[]) pincode);


        auto result = HiBON();
        result["recovery"] = Document(cast(HiBONT)wallet.wallet.toHiBON);
        result["pin"] = Document(cast(HiBONT)wallet.pin.toHiBON);
        result["account"] = Document(wallet.account.toHiBON);

        return Document(result).toResultDoc;
    }

import tagion.betterC.utils.Malloc;
    export uint first(const uint8_t* mem_ptr, const uint32_t mem_len)
    {
        set_memory(cast(void*) mem_ptr, cast(size_t) mem_len);
        // Recycle!Document recyclerDoc = cast(Recycle!Document) malloc(Recycle!Document.sizeof);
        return 0;
    }

    // export uint second(const uint8_t* mem_ptr, const uint32_t mem_len)
    // {
    //     set_memory(cast(void*) mem_ptr, cast(size_t) mem_len);
    //     Recycle!Document recyclerDoc = cast((Recycle!Document)*) malloc(Recycle!Document.sizeof);
    //     return 0;
    ...
    // export ResultDoc invoice_create(const ResultDoc doc, const ResultDoc dev_pin_doc,
