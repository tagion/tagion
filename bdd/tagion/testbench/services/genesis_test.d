...
class NetworkRunningWithGenesisBlockAndEpochChain {
    ...
    Document block() {
        ...
        check(signed_contract.contract.inputs.length == 1, format("should only contain one bill had %s", signed_contract
                .contract.inputs));
    ...
    Document run() @trusted {
        ...
        foreach (hist; histories.byKeyValue) {
            ...
            check(sorted_epochs.length <= sorted_heads.length, format("there should be equal or more heads than the total amount of epochs, heads %s, epochs %s", sorted_heads
                    .length, sorted_epochs.length));
            History sorted_hist;
        ...
        for (int i = 1; i < ref_epochs.length; i++) {
            ...
            if (ref_epoch.globals != prev_epoch.globals) {
                ...
                check(ref_epoch.globals.total < prev_epoch.globals.total, format("the total was not decreased previous %s current %s", prev_epoch
                        .globals.total, ref_epoch.globals.total));
