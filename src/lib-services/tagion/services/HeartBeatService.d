...
version (none) void heartBeatServiceTask(immutable(Options) opts) nothrow
in {
...
do {
    try {
        ...
        foreach (i, ref discovery_tid, ref dart_sync_tid, ref dart_tid; lockstep(discovery_tids,
                dart_sync_tids, dart_tids)) {
            //    foreach (i; 0..opts.nodes) {
            auto sync_opts = node_opts[cast(uint) i];

            auto p2pnode = new shared(p2plib.Node)(
                    "/ip4/0.0.0.0/tcp/" ~ to!string(sync_opts.port), 0);
            ...
            synchronized (master_net) {
                ...
                dart_sync_tid = spawn(&dartSynchronizeServiceTask!StdSecureNet,
                        sync_opts, p2pnode, shared_net, sector_range);
                //dart_sync_tids ~= dart_sync_tid;

                dart_tid = spawn(&dartServiceTask!StdSecureNet, sync_opts,
                        p2pnode, shared_net, sector_range);
                //dart_tids ~= dart_tid;
        ...
        do {
            ...
            if ((force_stop || abort) && live_counter <= 0)
                break;
        }
