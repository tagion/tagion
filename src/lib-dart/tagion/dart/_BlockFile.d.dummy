...
class BlockFile
{
    ...
    alias Index = Typedef!(ulong, ulong.init, "BlockIndex"); /// position in blockfile counted in blocks
    alias Size  = Typedef!(uint, uint.init, "Size");         /// size of a slice counted in blocks
    alias BlockFileStatistic = Statistic!(uint, Yes.histogram);
    static bool do_not_write;
    protected
        {
            File file;
            RecycleSegments recycle_segments;
    ...
    protected this(string filename, immutable uint block_size, const bool read_only = false)
        {
            File file;
            import std.stdio;
    ...
    protected this(File file, immutable uint block_size)
        {
            this.BLOCK_SIZE = block_size;
//            DATA_SIZE = block_size - Slice.CaptionSize;
            this.file = file;
            recycle_indices = Recycle(this);
            readInitial;
        }
    ...
    /**
       Used by the Inspect
    */
    protected this(immutable uint block_size) pure nothrow {
        this.BLOCK_SIZE = SIZE;
//        DATA_SIZE = BLOCK_SIZE - Block.HEADER_SIZE;
        //       recycle_segments = RecycleIndices(this);
    }
    ...
    static struct Segment
    {
        ...
        this(const Index index, const Size size) pure nothrow @nogc
        in
        {
            assert(size == 0);
        }
    ...
    /**
       Contains a list of recyclable segments
     */
    @safe
    struct RecycleSegments {
        alias SegmentSizes = RedBlackTree!(Segment*, (a, b) => a.size < b.size, true);
        alias Segments = RedBlackTree!(Segment*, (a, b) => (a.begin_index < b.begin_index) || (a.size < b.size));
        private {
            SegmentSizes segment_sizes;
            Segments segments;
        }
        this(BlockFile owner) pure nothrow {
            segment_sizes = new SegmentSizes;
            segments = new Segments;
        }


        void scrap(const Index index, const Size size) pure {
            auto segment = Segment(index, size);
    ...
    struct Slice {
        ...
        uint size;   /// Size of the slice counted in blocks
        uint previous_size; /// Size of the previous slice counted in blocks
        Buffer data; /// Payload data
        enum CaptionSize = size.sizeof+previous_size.sizeof+uint.sizeof;
        /// Size of the information stored before the payload data
    }

    void seek(const Index index) {
        file.seek(index*BLOCK_SIZE);
    }

    /**
       Map of a slice on the file

     */
    protected Slice read(const Index index) @trusted {
        uint data_size;
        Slice result = readCaption(index, data_size);
        .check(data_size <= result.size*BLOCK_SIZE-Slice.CaptionSize, format("Size of the payload exceeds the number of blocks reserved in slice @ %d", index));
        auto data = new ubyte[data_size*BLOCK_SIZE];
        file.rawRead(data);
        result.data = assumeUnique(data);
    ...
    /**
       Reads the caption of a slice with out the data
       index the position in the file counted in blocks
       This function outputs the data_size which is the size of the payload in bytes
       Returns:
       The slice without data
     */
    Slice readCaption(const Index index, out uint data_size) @trusted {
        Slice result;
        seek(index);
        ubyte[Slice.CaptionSize] _buf;
        auto buf = _buf[];
        file.rawRead(buf);
        result.size = buf.binread!uint;
        result.previous_size = buf.binread!uint;
        data_size = buf.binread!uint;
        return result;
    }

    protected void write(const(Slice) slice) @trusted
        in(slice.data.length <= slice.size*BLOCK_SIZE-Slice.CaptionSize)
        do {
        seek(slice.index);
        ubyte[Slice.CaptionSize] _buf;
        auto buf = _buf[];
        size_t pos;
        buf.binwrite(slice.size, &pos);
        buf.binwrite(slice.previous_size, &pos);
        buf.binwrite(cast(uint)slice.data.length, &pos);
        file.rawWrite(buf);
        file.rawWrite(slice.data);
    }

    /**
       Test for read and slice
     */
    unittest {
        immutable filename = fileid("slice").fullpath;
        auto blockfile = create(filename, "slice", SMALL_BLOCK_SIZE);
...
//            report(format("!!! recycle_indices %s", result.recycle_indices is null));
    ...
    //     struct BlockInfo {
    //         uint previous; /// Points to the previous block
    //         uint next; /// Points to the next block
    //         uint size; /// size of the data in this block
    //         uint index; /// Block index
    //         bool head; /// Set to `true` this block starts a chain of blocks
    //     }
    //     Block block;
    //     uint index;
...
version(none) {
           console.writef("%d [ ", index);
            scope(exit) {
                ...
                console.writefln("recycled_to_save= %s", recycle_indices.recycled_blocks_which_needs_to_be_saved.keys.sort);
            }
            ...
            if (!block_range.empty) {
                ...
                bool ok;
                const remove_list = range
                    .map!(block => tuple!("block", "index")(block, range.index))
                    .tee!(a => ok &= (a.block !is null) && !recycle_indices.isRecyclable(a.index))
    ...
    void store()
        {
            ...
            // catch (Exception e) {
            //     void __check() nothrow @trusted {
            //         import std.exception;
            //         assumeWontThrow(console.writeln(e));
            //     }
            //     __check;
            //     // }
...
version (unittest)
{
    ...
    private const(Basic.FileNames) fileid(string prefix = null) @safe
    {
